...
/**
 * Main class for Plan's Forge version.
 *
 * @author Kopo942
 */
@Mod("plan")
public class PlanForge implements PlanPlugin {
    ...
    public PlanForge() {
        ...
        MinecraftForge.EVENT_BUS.addListener(event -> {
            if (event instanceof ServerStartingEvent startingEvent) {
                this.server = startingEvent.getServer();
                onEnable();
            }
        });
        MinecraftForge.EVENT_BUS.addListener(event -> {
            if (event instanceof RegisterCommandsEvent commandsEvent) {
                commandManager = new CommandManager(commandsEvent.getDispatcher(), this, errorLogger);
            }
        });
        MinecraftForge.EVENT_BUS.addListener(event -> {
            if (event instanceof ServerStoppingEvent stoppingEvent) {
                onDisable();
            }
        });
    }
